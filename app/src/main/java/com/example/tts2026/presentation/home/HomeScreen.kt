...
fun HomeRoute(
    ...
    // Collect UI state gom users, cars, email va selectedTab.
    val uiState by viewModel.uiState.collectAsStateWithLifecycle()
    // Collect one-time event logout rieng voi state du lieu.
    val logoutCompleted by viewModel.logoutCompleted.collectAsStateWithLifecycle()

    LaunchedEffect(logoutCompleted) {
        // Side effect: chi navigate khi ViewModel bao logout da hoan tat.
        if (logoutCompleted) {
...
fun HomeScreen(
    ...
    // State cuc bo cua UI: dialog nao dang mo, khong can dua vao ViewModel.
    ...
    Scaffold(
        ...
        bottomBar = {
            NavigationBar {
                NavigationBarItem(
                    ...
                    // Event di nguoc ve ViewModel de luu selected tab vao DataStore.
        ...
        Column(
            ...
            when (uiState.selectedTab) {
                // View chi doc state va chon content tuong ung voi tab hien tai.
